...
| TensorRT failure on one frame | recoverable | Clear detector health and the current detection result, publish an unavailable event, forward the original frame to the optional annotated output, and try the next frame; `AnnotationDetector.RepeatedInferenceFailuresInvalidateLatestDetections`. |
...
| Encoder pipeline or appsrc failure | restart-module | Close the failed pipeline and rebuild for the next valid frame. Keep AU PTS increasing across the rebuild and mark its first AU as discontinuous; `H264Encoder.FailedPipelineRebuildKeepsTimelineAndMarksNextAccessUnit`. |
