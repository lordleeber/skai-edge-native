...
libdatachannel and skai-ice. They bind ephemeral loopback ports and keep YAML,
...
The portable detector is compiled only into the test executable. It locates the
white pixels in the decoded frame and provides a known class and bounding box.
It is a controlled substitute for GPU inference, so this test checks module
composition and persistence rather than YOLO accuracy. The Jetson variant uses
...
no file-input path or detector override is added to the production executable.
...
The tests exercise existing production lifecycle adapters rather than launching
